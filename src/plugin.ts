...
function resolvePostHogBaseUrl(baseUrl) {
  ...
  if (
    !POSTHOG_BUILTIN_ALLOWED_HOSTS.has(host) &&
...
function buildAuthorizeUrl({ auth, input }) {
  ...
  url.searchParams.set(
    "redirect_uri",
    requireString(input.redirectUri, "redirectUri"),
  );
...
function normalizeSearchQuery(value) {
  ...
  return (
    unwrapFencedMarkdownCode(trimmed) ??
    unwrapInlineMarkdownCode(trimmed) ??
    trimmed
  );
}
...
const plugin: DesktopCodePlugin = {
  ...
  version: "0.1.0",
  description:
    "Queries PostHog organizations, projects, exception issues, and events.",
  metadata: {
    dataSource: {
      sourceType: "posthog",
      setupFields: [
  ...
  actions: [
    ...
    },
    {
      ...
      description:
        "List PostHog projects, optionally scoped to an organization.",
      riskLevel: "read",
      fields: [
        {
          key: "orgSlug",
    ...
    },
    {
      ...
      description:
        "List exception fingerprints bounded by last-seen timestamps.",
      riskLevel: "read",
      fields: [
        {
          key: "orgSlug",
